            ...
            if not sessions_df.empty:
                ...
                with col1:
                    selected_session = st.selectbox(
                        ...
                        format_func=lambda x: f"Session {x} - {sessions_df[sessions_df['id']==x]['username'].iloc[0]} - {pd.to_datetime(sessions_df[sessions_df['id']==x]['session_date'].iloc[0]).strftime('%Y-%m-%d %H:%M')}"
                    )
                ...
                with col1:
                    ...
                    st.markdown(f'<div class="metric-value">{session_data["max_velocity"]:.3f} m/s</div>', unsafe_allow_html=True)
                    st.markdown('<div class="metric-label">Peak speed achieved</div>', unsafe_allow_html=True)
                    st.markdown('</div>', unsafe_allow_html=True)

                with col2:
                    st.markdown('<div class="metric-card">', unsafe_allow_html=True)
                    st.markdown(f'<div class="metric-label">Avg Velocity</div>', unsafe_allow_html=True)
                    st.markdown(f'<div class="metric-value">{session_data["avg_velocity"]:.3f} m/s</div>', unsafe_allow_html=True)
                ...
                with col3:
                    ...
                    st.markdown(f'<div class="metric-value">{session_data["total_distance"]:.1f} m</div>', unsafe_allow_html=True)
                ...
                with col4:
                    ...
                    st.markdown(f'<div class="metric-value">{session_data["analysis_time"]:.3f} s</div>', unsafe_allow_html=True)
                    st.markdown('<div class="metric-label">Total analysis duration</div>', unsafe_allow_html=True)
                    st.markdown('</div>', unsafe_allow_html=True)
        ...
        with tab_reports:
            ...
            if not sessions_df.empty:
                ...
                with st.container():
                    ...
                    st.markdown("Comprehensive analysis including position, velocity, and timing data for all ranges.")
                    ...
                    with col2:
                        ...
                        if st.button("📥 Export to Excel", use_container_width=True, type="primary"):
                            with st.spinner("Generating report..."):
                                ...
                                session_data = sessions_df[sessions_df['id'] == selected_session_report].iloc[0].to_dict()
                                performance_df = pd.read_sql_query("""
                                                                   SELECT * FROM performance_data
                                                                   WHERE session_id = ?
                                                                   ORDER BY time_stamp
                                                                   """, conn, params=(selected_session_report,))
                    ...
                    with col1:
                        ...
                        st.write(f"- Date: {pd.to_datetime(preview_session['session_date']).strftime('%Y-%m-%d %H:%M')}")
                    ...
                    st.info("📄 The Excel report includes detailed data for all segments, performance charts, and comprehensive analysis.")
            else:
            ...
            with tab_users:
                ...
                with col1:
                    ...
                    with st.form("create_user_form"):
                        ...
                        if new_role == "runner":
                            # Get list of coaches
                            conn = sqlite3.connect('running_analytics.db')
                            coaches_df = pd.read_sql_query("SELECT id, username FROM users WHERE role = 'coach'", conn)
                            conn.close()
                            ...
                            if not coaches_df.empty:
                                coach_id = st.selectbox(
                                    ...
                                    format_func=lambda x: coaches_df[coaches_df['id']==x]['username'].iloc[0]
                                )
                            else:
                ...
                with col2:
                    ...
                    users_df = pd.read_sql_query("""
                                                 SELECT u1.id, u1.username, u1.role, u2.username as coach_name, u1.created_at
                                                 FROM users u1
                ...
                total_sessions = pd.read_sql_query("SELECT COUNT(*) as count FROM sessions", conn).iloc[0]['count']

                col1, col2, col3, col4 = st.columns(4)

                with col1:
                    admin_count = pd.read_sql_query("SELECT COUNT(*) as count FROM users WHERE role = 'admin'", conn).iloc[0]['count']
                    st.metric("Admins", admin_count)

                with col2:
                    coach_count = pd.read_sql_query("SELECT COUNT(*) as count FROM users WHERE role = 'coach'", conn).iloc[0]['count']
                    st.metric("Coaches", coach_count)

                with col3:
                    runner_count = pd.read_sql_query("SELECT COUNT(*) as count FROM users WHERE role = 'runner'", conn).iloc[0]['count']
